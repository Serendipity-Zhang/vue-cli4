...
<template>
  <div class="main">
    <div class="panel panel-info" style="margin:20px;">
      <!-- 头部 text-->
      <div class="panel-headingsdd">
        <h1>vue定制学习计划</h1>
        <div>
          <a
            >共有 <span class="badge">{{ items.length }}</span
            >个计划,</a
          >
          <a
            >已完成：<span class="badge">{{ completed }}</span></a
          >
          <a
            >未完成：<span class="badge">{{
    ...
    justify-content: space-between;"
        >
          ...
          <span class="input-group-btn">
            <button class="btn btn-primary" @click="add()" :disabled="!input">
              添加计划
            </button>
          </span>
        </div>
        <div class="btn-groups btn-group-justifieds">
          <a
            class="btn btn-defaults"
            :class="cur == 1 ? 'selecteds' : ''"
            @click="filterItem(1)"
            >所有计划</a
          >
          <a
            class="btn btn-default"
            :class="cur == 2 ? 'selected' : ''"
            @click="filterItem(2)"
            >已完成</a
          >
          <a
            class="btn btn-default"
            :class="cur == 3 ? 'selected' : ''"
            @click="filterItem(3)"
            >未完成</a
          >
        </div>
