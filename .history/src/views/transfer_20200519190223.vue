      ...
      <div class="city-contaion-con city-contaion-left">
        <ul>
           <li v-for="(city,i) in leftCitys" 
            :key="i" @click="AddCity(city)">{{city.name}}</li>
        </ul>
      </div>
    </div>
        ...
        <p>城市12 / 当前选中\thyyt</p>
                   ...
                   <div class="city-contaion-con">
                            <ul>
                                ...
                                <li v-for="(city,i) in rightCitys" :key="i" @mouseover="XFlag=i" @mouseout="XFlag='hidde'">
                                    <i class="iconfont icon-left icon-cha icon-close-city" v-show="XFlag == i" @click="RemoveCity(city.id,i)"></i>
                                    {{city.name}}
                                </li>
